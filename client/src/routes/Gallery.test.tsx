...
// jsdom has no IntersectionObserver — stub one that fires immediately on observe().
...
describe("Gallery", () => {
  ...
  it("renders a thumbnail per photo", async () => {
    (fetch as ReturnType<typeof vi.fn>).mockResolvedValue(
      jsonResponse({ photos: [photo(2), photo(1)], nextCursor: null }),
    );
  ...
  it("opens a lightbox with the full image and a link to the event", async () => {
    (fetch as ReturnType<typeof vi.fn>).mockResolvedValue(
      jsonResponse({ photos: [photo(3)], nextCursor: null }),
    );
  ...
  it("labels an orphaned photo as not linked to an event", async () => {
    (fetch as ReturnType<typeof vi.fn>).mockResolvedValue(
      jsonResponse({ photos: [photo(4, { log: null, logId: null })], nextCursor: null }),
    );
  ...
  it("confirms then deletes a photo via the gallery endpoint", async () => {
    ...
    fetchMock.mockImplementation((url: string, init?: RequestInit) => {
      if (init?.method === "DELETE") return Promise.resolve(jsonResponse(undefined, 204));
      return Promise.resolve(jsonResponse({ photos: [photo(5)], nextCursor: null }));
    });
  ...
  it("loads the next page when the sentinel scrolls into view", async () => {
    ...
    fetchMock.mockImplementation((url: string) => {
      if (url.includes("cursor=")) {
        return Promise.resolve(jsonResponse({ photos: [photo(10)], nextCursor: null }));
      }
      return Promise.resolve(jsonResponse({ photos: [photo(12), photo(11)], nextCursor: 11 }));
    });
    ...
    await waitFor(() => {
      expect(fetchMock).toHaveBeenCalledWith(
        expect.stringContaining("cursor=11"),
        expect.anything(),
      );
    });
