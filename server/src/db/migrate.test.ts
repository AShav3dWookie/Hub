...
describe("migrations", () => {
  ...
  it("0009 creates app_settings (plain table, no triggers, not in the change feed)", () => {
    const db = runMigrations(tmpDb());
    const c = db.$client;

    const cols = (c.prepare("PRAGMA table_info(app_settings)").all() as Array<{
      name: string;
      notnull: number;
      pk: number;
    }>).reduce<Record<string, { notnull: number; pk: number }>>((acc, col) => {
    ...
    expect(cols).toMatchObject({
      key: { pk: 1 },
      value: { notnull: 1 },
      updated_at: { notnull: 1 },
    });

    // Server-only settings (the login password hash): never replicated to clients.
    const triggers = (
      c.prepare("SELECT name FROM sqlite_master WHERE type = 'trigger'").all() as Array<{
        name: string;
      }>
